...
public class ApiClient {
    ...
    private void handleClientError(String url, ClientHttpResponse httpResponse) {
        try {
            ...
            throw new com.whoshot.exception.ApiClientException(
                    "Client error: " + httpResponse.getStatusCode() + " for URL: " + url);
        } catch (Exception e) {
            throw new com.whoshot.exception.ApiClientException("Error handling client error response", e);
        }
    }
    ...
    private void handleServerError(String url, ClientHttpResponse httpResponse) {
        try {
            ...
            throw new com.whoshot.exception.ApiClientException("Error handling server error response", e);
        }
    }
     ...
     * @throws com.whoshot.exception.ApiClientException if the request fails after all retries
     */
    @Retryable(
            retryFor = {com.whoshot.exception.ApiClientException.class},
            backoff = @Backoff(delay = 1000, multiplier = 2)
    ...
    public <T> T get(String url, ParameterizedTypeReference<T> typeRef, Map<String, String> headers) {
        try {
            ...
            if (response == null) {
                throw new com.whoshot.exception.ApiClientException("Received null response from: " + url);
            }

            log.debug("GET request successful: {}", url);
            return response;

        } catch (com.whoshot.exception.ApiClientException e) {
            throw e;
        } catch (Exception e) {
            log.error("Unexpected error during GET request to {}: {}", url, e.getMessage(), e);
            throw new com.whoshot.exception.ApiClientException("Unexpected error for URL: " + url, e);
        }
    }
